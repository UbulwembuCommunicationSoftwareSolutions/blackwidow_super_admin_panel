...
/**
 * JSON API for the CRM (auth: Sanctum personal access token with `crm` ability,
 * issued via `php artisan crm:create-token`).
 *
 * Mirrors the customer + customer-subscription surface of {@see McpSiteController},
 * with the same secret-hiding rules:
 *  - Customer:            hides token / google_api_key / S3 credentials.
 *  - CustomerSubscription: always hides database_password; hides `env` unless ?include_env=1.
 */
class CrmController extends Controller
{
    /** @var list<string> */
    private const CUSTOMER_HIDDEN = [
        'token',
        'google_api_key',
        's3_endpoint',
        's3_key',
    ...
    {
        $validated = $this->crmListRules($request);
        ...
        $paginator->getCollection()->each(
            fn (Customer $c) => $c->makeHidden(self::CUSTOMER_HIDDEN)
        );
    ...
    public function showCustomer(int $id): JsonResponse
    {
        ...
        $row->makeHidden(self::CUSTOMER_HIDDEN);
    ...
    public function storeCustomer(Request $request): JsonResponse
    {
        $validated = McpMutationHelper::onlyCustomerSafe(
            $request->validate($this->customerStoreRules())
        );
        $row = Customer::query()->create($validated);

        return response()->json(['data' => $row->fresh()->makeHidden(self::CUSTOMER_HIDDEN)], 201);
    }
    ...
    public function updateCustomer(Request $request, int $id): JsonResponse
    {
        $row = Customer::query()->findOrFail($id);
        $validated = McpMutationHelper::onlyCustomerSafe(
            $request->validate($this->customerUpdateRules())
        );
        if ($validated !== []) {
            $row->update($validated);
        }

        return response()->json(['data' => $row->fresh()->makeHidden(self::CUSTOMER_HIDDEN)]);
    }
    ...
    {
        $validated = $this->crmListRules($request, extra: [
     ...
     */
    private function customerStoreRules(): array
    {
        return array_merge(
            [
                'company_name' => ['required', 'string', 'max:255'],
            ],
            $this->customerFieldRules('nullable')
        );
    }
    ...
    {
        $rules = $this->customerFieldRules('sometimes');
     ...
     * @return array<string, mixed>
     */
    private function customerFieldRules(string $w): array
    {
        return [
     ...
     * @return array<string, mixed>
     */
    private function crmListRules(Request $request, array $extra = []): array
    {
        $rules = array_merge([
