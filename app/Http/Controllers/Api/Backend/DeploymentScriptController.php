...
{
    /** @var list<string> */
    private const SEARCHABLE = ['script', 'customerSubscription.url'];

    /** @var list<string> */
    private const SORTABLE = ['id', 'customer_subscription_id', 'created_at', 'updated_at'];

    public function index(Request $request): JsonResponse
    {
        $this->authorize('viewAny', DeploymentScript::class);

        $validated = $this->listFilters($request, [
            'customer_subscription_id' => ['sometimes', 'integer', 'exists:customer_subscriptions,id'],
        ], self::SORTABLE);

        $query = DeploymentScript::query()->with('customerSubscription:id,url');
    ...
    {
        $row = DeploymentScript::query()->findOrFail($id);
    ...
    {
        $this->authorize('create', DeploymentScript::class);

        $row = DeploymentScript::query()->create($request->validate([
